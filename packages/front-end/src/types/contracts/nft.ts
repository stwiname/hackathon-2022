import { MyNFT__factory, MyNFT, RentToken, RentToken__factory, Renting, Renting__factory } from "@psst/contracts";
...
export class RentNFTSdk {
    ...
    constructor(signerOrProvider: Signer | Provider) {
        ...
        this.rentContract = Renting__factory.connect(RENT_ADDRESS, signerOrProvider)
    }
}
...
export async function rentNFTChecked(sdk: RentNFTSdk, accountAddress: string, tokenId: BigNumberish) {

    const price = await sdk.rentContract.priceOf(tokenId);

    const allowance = await sdk.erc20Contract.allowance(accountAddress, sdk.rentContract.address);

    if (price.gt(allowance)) {
        // Submit approval tx
        const approveTx = await sdk.erc20Contract.approve(sdk.rentContract.address, price)
...
export async function getAllNftDetails(sdk: RentNFTSdk, tokenId: BigNumberish): Promise<NFTMetadata & { owner: string, price: BigNumberish }> {
